...
extension _MediaDetailActionButtons on _MediaDetailScreenState {
  Widget _buildActionButtons(MediaItem metadata) {
    ...
    final playActionLabel = playIcon == PhosphorIconsFill.play ? t.common.resume : t.common.play;
    ...
    final trailerAction = primaryTrailer == null
        ...
        : FocusableAction(
            ...
            builder: (context, state) => iconActionButton(
              state,
              onPressed: onPlayTrailer,
              icon: const AppIcon(PhosphorIconsFill.filmSlate, fill: 1),
    ...
    final shuffleAction = (metadata.isShow || metadata.isSeason)
        ? FocusableAction(
            ...
            builder: (context, state) => iconActionButton(
              ...
              icon: const AppIcon(PhosphorIconsFill.shuffle, fill: 1),
        ...
        : FocusableAction(
            ...
            builder: (context, state) => KeyedSubtree(
              ...
              child: iconActionButton(
                ...
                icon: AppIcon(
                  (onWatchlist ?? false) ? PhosphorIconsFill.bookmark : PhosphorIconsFill.bookmarkSimple,
                  fill: 1,
                ),
    ...
    final rateAction = widget.isOffline
        ...
        : FocusableAction(
            ...
            builder: (context, state) => iconActionButton(
              state,
              onPressed: () => _showRatingDialog(context, metadata),
              icon: AppIcon(metadata.isFavorite == true ? PhosphorIconsFill.heart : PhosphorIconsRegular.heart),
        ...
        for (final candidate in candidates)
          AppMenuItem(
            ...
            trailing: (candidate.source.isOnWatchlist(metadata.kind, candidate.ids) ?? false)
                ? const AppIcon(PhosphorIconsFill.bookmark, fill: 1)
                : const AppIcon(PhosphorIconsFill.bookmarkSimple),
          ),
      ],
    );
  ...
  Widget _buildWatchedToggleButton(
    ...
    return IconButton.filledTonal(
      onPressed: () => unawaited(_handleWatchedTogglePressed(metadata)),
      icon: AppIcon(metadata.isWatched ? PhosphorIconsFill.checkCircle : PhosphorIconsFill.check, fill: 1),
    ...
    return Consumer<DownloadProvider>(
      builder: (context, downloadProvider, _) {
        ...
        if (progress?.status == DownloadStatus.paused) {
          return IconButton.filledTonal(
            onPressed: () => unawaited(_handleDownloadButtonPressed(metadata)),
            icon: const AppIcon(PhosphorIconsFill.pauseCircle, fill: 1),
        ...
        if (progress?.status == DownloadStatus.failed) {
          return IconButton.filledTonal(
            onPressed: () => unawaited(_handleDownloadButtonPressed(metadata)),
            icon: const AppIcon(PhosphorIconsFill.warningCircle, fill: 1),
        ...
        if (progress?.status == DownloadStatus.partial) {
          ...
          if (hasSyncRule) {
            ...
            return IconButton.filledTonal(
              onPressed: () => unawaited(_handleDownloadButtonPressed(metadata)),
              tooltip: tooltip,
              icon: AppIcon(isEnabled ? PhosphorIconsFill.arrowsClockwise : PhosphorIconsFill.cloudSlash, fill: 1),
              iconSize: iconSize,
