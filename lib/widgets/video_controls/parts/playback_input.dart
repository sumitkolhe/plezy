...
extension _PlayerControlsPlaybackInputMethods on _PlayerControlsState {
  ...
  void _onRateChanged(double newRate) {
    ...
    final icon = newRate >= 1.0 ? PhosphorIconsFill.fastForward : PhosphorIconsFill.filmStrip;
  ...
  void _seekToChapterWithFeedback({required bool forward}) {
    ...
    if (resolved.target != null) {
      ...
      widget.toastController.show(
        forward ? PhosphorIconsFill.skipForward : PhosphorIconsFill.skipBack,
  ...
  Widget _buildSpeedIndicator() => const PlayerToastIndicator(icon: PhosphorIconsFill.fastForward, text: '2x');
}
