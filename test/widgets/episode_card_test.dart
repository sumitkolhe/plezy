...
void main() {
  ...
  testWidgets('the details sheet offers the primary actions as circles under Play', (tester) async {
    ...
    for (final label in [t.mediaMenu.rate, t.downloads.downloadNow, t.mediaMenu.fileInfo, t.tooltips.moreOptions]) {
      expect(
        find.descendant(of: sheet, matching: find.byTooltip(label)),
        findsOneWidget,
        reason: label,
      );
    }
    ...
    expect(tester.getTopRight(find.byTooltip(t.tooltips.moreOptions)).dx, closeTo(tester.getTopRight(play).dx, 0.01));
