...
void main() {
  ...
  testWidgets('keyboard keeps equals on main page and exposes symbols page', (tester) async {
    ...
    expect(find.byIcon(PhosphorIconsFill.mathOperations), findsOneWidget);

    await tester.tap(find.byIcon(PhosphorIconsFill.mathOperations));
    await tester.pumpAndSettle();
  ...
  testWidgets('rendered backspace deletes whole graphemes and keeps the keyboard open', (tester) async {
    ...
    for (final grapheme in ['😀', 'e\u0301', '🇯🇵', '👨‍👩‍👧‍👦']) {
      ...
      await tester.tap(find.byIcon(PhosphorIconsFill.backspace));
      await tester.pump();
